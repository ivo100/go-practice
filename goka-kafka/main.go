...
func runEmitter() {
	log.Printf("==== running emitter, topic=%s", topic)
	...
	for i := 0; i < 5; i++ {
		msg := fmt.Sprintf("my-value-B-%d", i)
		key := "my-key-B"
		err = emitter.EmitSync(key, msg)
		if err != nil {
			log.Fatalf("error emitting message: %v", err)
		}
		log.Printf("key %s, message %s sent", key, msg)
	}
}
...
func runProcessor() {
	...
	cb := func(ctx goka.Context, msg interface{}) {
		...
		log.Printf("Received key = %s, counter = %v, msg = %v", ctx.Key(), counter, msg)
	}
...
func main() {
	...
	log.Printf("Starting goka with config: %+v", config)
